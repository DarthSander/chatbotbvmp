...
# ───────── Dummy default topics met korte uitleg ─────────
...
def summarize_chunk(chunk: List[dict]) -> str:
    ...
    r = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "system", "content": "Vat samen in max 300 tokens."},
...
# ───────── Tool-implementaties ─────────
def _set_theme_options(session_id: str, options: List[dict]) -> str:
...
def _set_topic_options(session_id: str, theme: str, options: List[dict]) -> str:
    """`options` = [{'name','description'}, …]"""
    st = get_session(session_id)
...
def _complete_theme(session_id: str) -> str:
    """Gebruiker klaar met onderwerpen."""
    st = get_session(session_id)
    if len(st["themes"]) >= 6:
        st["stage"] = "qa"
    else:
...
def _log_answer(session_id: str, theme: str, question: str, answer: str) -> str:
    get_session(session_id)["qa"].append({"theme": theme,
                                          "question": question,
                                          "answer": answer})
    persist(session_id)
    return "ok"
...
BASE_AGENT = Agent(
    ...
    model="gpt-4.1",
    instructions=(
        "Je helpt ouders hun geboorteplan vorm te geven (je bent géén "
        "digitale verloskundige).\n\n"
        "• Start met `set_theme_options` en stuur max 6 objecten "
        "`{name, description}` voor de keuzeknoppen.\n"
        "• Na klik roept de UI `register_theme`. Stuur direct daarna "
        "`set_topic_options` met **minimaal** 4 items "
        "`{name, description}` behorend bij dat thema.\n"
        "• De UI laat ouders max 4 onderwerpen kiezen; voor ieder gekozen "
        "onderwerp roept zij `register_topic`.\n"
        "• Zodra de ouder op de knop “Door …” drukt, roept de UI "
        "`complete_theme`. Als er 6 thema’s zijn, gaat je fase naar 'qa'.\n"
        "• In de QA-fase stel je verdiepende vragen en sla je de antwoorden op "
        "met `log_answer`.\n\n"
        "Gebruik altijd het juiste `session_id`. Antwoord in het Nederlands."
    ),
    tools=[
        set_theme_options, set_topic_options, register_theme,
        register_topic, complete_theme,
        log_answer, update_answer, get_state_tool
    ],
)

# ───────── Streaming /chat (optioneel) ─────────
def stream_run(tid: str):
    with client.beta.threads.runs.stream(thread_id=tid,
                                         assistant_id=ASSISTANT_ID) as ev:
        for e in ev:
