...
def agent_route():
    ...
    if command == "initialize":
        ...
        if not topic: abort(404, "Topic niet gevonden")
        plan_summary = "\n".join(f"- Thema '{t['name']}', Vraag '{top['question']}', Antwoord: '{top.get('answer', '')}'" for t in st['plan'] for top in t['topics'] if top.get('answer') and top['id'] != topic_id and top.get('answer') != '__SKIPPED__') or "Nog geen andere antwoorden gegeven."
